...
// init Masonry
    ...
    const commentBtn = $("#viewComment input[type=button]");

    commentBtn.on('click',function(){

        const imgid = $("#pir_img_id");
        const commentmsg = $("#viewComment input[type=text]");

        if(!commentmsg.val()){
            alert("댓글을 입력해주세요!");
            commentmsg.trigger('focus');
            return false;
        }

        const fd = new FormData();
        fd.append('imgid',String(imgid.val()));
        fd.append('msg', String(commentmsg.val()));


        $.ajax({
            url: '//pirare.jupiterflow.com/modules/comment/api/writeComment.php', // url where upload the image
            type: 'POST',
            data: fd,
            dataType: 'json',
            success: function (data) {
